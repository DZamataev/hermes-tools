...
[[ "$(/usr/libexec/PlistBuddy -c 'Print :ProgramArguments:1' "$PLIST")" == "$ROOT/hermes-webui/start.sh" ]] ||
  fail "installed plist must point to the in-repository Hermes WebUI checkout"
...
[[ "$(/usr/libexec/PlistBuddy -c 'Print :WorkingDirectory' "$PLIST")" == "$ROOT/hermes-webui" ]] ||
  fail "installed plist must use the in-repository working directory"
